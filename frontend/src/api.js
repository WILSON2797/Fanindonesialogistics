// ============================================
// API Wrapper — fetch helper for PHP backend (ESM)
// ============================================

// Use Vite's environment variable for base URL instead of guessing from the current URL
// Guessing from pathname is dangerous because in SPA, pathname is the route (e.g. /about)
let basePath = import.meta.env.BASE_URL || '/';

// Ensure basePath doesn't end with a slash, and if it's exactly '/', make it empty ''
// so that when we do basePath + '/api', it becomes '/api' instead of '//api' (which browser treats as a domain)
if (basePath === '/') {
  basePath = '';
...
// For Laravel/Laragon subfolder compatibility in production (if base is ./)
if (basePath === '.') {
  basePath = '';
}
...
export const api = {
  ...
  getWarehouses() { return this.get('/warehouse'); },
  createWarehouse(d) { 
    return (d instanceof FormData) ? this.upload('/warehouse', d) : this.post('/warehouse', d); 
  },
  updateWarehouse(id, d) { 
    return (d instanceof FormData) ? this.upload(`/warehouse/${id}`, d, 'POST') : this.put(`/warehouse/${id}`, d); 
  },
  deleteWarehouse(id) { return this.del(`/warehouse/${id}`); },
  getWarehouseById(id) { return this.get(`/warehouse/${id}`); },
  getWarehouseGallery(id) { return this.get(`/warehouse/${id}/gallery`); },
  addWarehouseGallery(warehouseId, fd) { return this.upload(`/warehouse/${warehouseId}/gallery`, fd); },
  deleteWarehouseGallery(galleryId) { return this.del(`/warehouse/gallery/${galleryId}`); },
  getWarehouseHighlights(id) { return this.get(`/warehouse/${id}/highlights`); },
  addWarehouseHighlight(warehouseId, data) { return this.post(`/warehouse/${warehouseId}/highlights`, data); },
  deleteWarehouseHighlight(highlightId) { return this.del(`/warehouse/highlights/${highlightId}`); },
